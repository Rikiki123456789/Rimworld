...
namespace OutpostGenerator
{
    ...
    // TODO: create MiningCo. backgrounds.
    ...
    public static class OG_BigOutpost
    {
        ...
        static void EnsureAtLeastOneMedibayInOutpost()
        {
            ...
            {
                Log.Warning("MiningCo. Outpost generator: EnsureAtLeastOneMedibayInOutpost did not found any medium room.");
                return;
            }
        ...
        private static void ReconnectEverything()
        {
            foreach (Thing thing in outpostData.outpostThingList)
            {
                CompPower powerComp = thing.TryGetComp<CompPower>();
                if (powerComp != null)
                {
                    if ((thing.def == OG_Util.ForceFieldGeneratorDef)
                        || (powerComp is CompPowerPlant)
                        || (powerComp is CompPowerTransmitter)
                        || (powerComp is CompPowerBattery))
                    {
                        continue;
                    }
                    PowerConnectionMaker.DisconnectFromPowerNet(powerComp);
                    PowerConnectionMaker.TryConnectToAnyPowerNet(powerComp);
                }
            }
        }

        private static void SpawnAreaGenerator(IntVec3 areaSouthWestOrigin)
        {
            Building_AreaGenerator areaGenerator = ThingMaker.MakeThing(ThingDef.Named("AreaGenerator")) as Building_AreaGenerator;
            areaGenerator.areaSouthWestOrigin = areaSouthWestOrigin;
            GenSpawn.Spawn(areaGenerator, areaSouthWestOrigin + new IntVec3(-1, 0, -1));
        }
    }
}
