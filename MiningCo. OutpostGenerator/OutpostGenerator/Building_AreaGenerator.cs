...
{
    /// <summary>
    /// Building_AreaGenerator class.
    /// </summary>
    /// <author>Rikiki</author>
    /// <permission>Use this code as you want, just remember to add a link to the corresponding Ludeon forum mod release thread.
    /// Remember learning is always better than just copy/paste...</permission>
    public class Building_AreaGenerator : Building
    {
            ...
            if (Find.TickManager.TicksGame == 1)
            {
                ...
                if (outpostArea != null)
                {
                    for (int xOffset = 0; xOffset < OG_BigOutpost.areaSideLength; xOffset++)
                    {
                        ...
                        {
                            outpostArea.Set(areaSouthWestOrigin + new IntVec3(xOffset, 0, zOffset));
                        }
                    }
                }

                // Initialize no roof area cells.
                for (int xOffset = 0; xOffset < OG_BigOutpost.areaSideLength; xOffset++)
                {
                    for (int zOffset = 0; zOffset < OG_BigOutpost.areaSideLength; zOffset++)
                    {
